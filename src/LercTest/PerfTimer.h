...
public:
  ...
  long ms() const;       // milli seconds (convenient: get ms or us as long)
  long us() const;       // micro seconds
...
protected:
  ...
  long convert( double factor ) const;
...
inline long PerfTimer::ms() const
{
  return convert(1000);
}
...
inline long PerfTimer::us() const
{
...
inline long PerfTimer::convert(double fac) const
{
  double sec = this->sec();
  if (sec >= 0)
  {
    double d = sec * fac + 0.5;
    if (d <= 2147483647)           // max long
      return (long)d;
  }
  return -1;
}
